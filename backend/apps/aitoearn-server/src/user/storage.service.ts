...
import { AddUsedStorageDto, DeductUsedStorageDto } from './dto/storage.dto'
...
export class StorageService {
  ...
  async addUsedStorage(data: AddUsedStorageDto): Promise<void> {
    ...
    await this.userRepository.updateById(
      data.userId,
      { $set: { usedStorage: newUsedStorage } },
    )
    // 清除缓存
    this.redisService.del(`UserInfo:${data.userId}`)
  }
  ...
  async setTotalStorage(userId: string, totalStorage: number, expiredAt?: Date): Promise<void> {
    ...
    await this.userRepository.setTotalStorage(userId, totalStorage, expiredAt)
