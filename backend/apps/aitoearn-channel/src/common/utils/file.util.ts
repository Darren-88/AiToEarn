...
export async function streamDownloadAndUpload(
  url: string,
  uploadHandler: (upData: Buffer, partNumber: number) => Promise<any>,
  postUploadHandler: (partCount: number) => Promise<any>,
  ...
  try {
    ...
    for (let start = 0; start < totalSize; start += chunkSize) {
      ...
      await uploadHandler(buffer, partNumber)
      partNumber++
    }
    await postUploadHandler(partNumber)
  }
  catch (e) {
    throw new Error(e)
  }
