...
export class EngagementController {
  ...
  @Post('channel/engagement/meta/post/detail')
  async getMetaPostDetail(@Body() data: { accountId: string, platform: string, postId: string }) {
    const res = await this.engagementService.getMetaPostDetail(data.accountId, data.platform, data.postId)
    return res
  }
