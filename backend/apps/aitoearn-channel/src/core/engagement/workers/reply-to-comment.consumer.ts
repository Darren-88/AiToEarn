...
@Processor(QueueName.EngagementReplyToComment, {
  concurrency: 3,
  stalledInterval: 15000,
  maxStalledCount: 1,
})
export class EngagementReplyToCommentConsumer extends WorkerHost implements OnModuleDestroy {
  ...
  async onModuleDestroy() {
    this.logger.log('EngagementReplyToCommentConsumer is being destroyed, closing worker...')
