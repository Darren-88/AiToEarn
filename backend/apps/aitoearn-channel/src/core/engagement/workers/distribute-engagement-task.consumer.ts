...
@Processor(QueueName.EngagementTaskDistribution, {
  concurrency: 3,
  stalledInterval: 15000,
  maxStalledCount: 1,
})
export class EngagementTaskDistributionConsumer extends WorkerHost implements OnModuleDestroy {
  ...
  async onModuleDestroy() {
    this.logger.log('EngagementTaskDistributionConsumer is being destroyed, closing worker...')
