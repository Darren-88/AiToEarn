...
import { PostDatas as PostModel, UserTaskPosts } from '../schemas'
...
export class PostRepository extends BaseRepository<PostModel> {
  ...
  /**
   * 从 user_tasks_posts 集合中获取数据
   * @param payload 查询参数
   * @param payload.accountId 账号ID（可选）
   * @param payload.platform 平台类型（可选）
   * @param payload.uid 平台用户唯一ID（可选）
   * @param payload.taskId 任务ID（可选）
   * @param payload.userId 用户ID（可选）
   * @param payload.postId 作品ID（可选）
   * @returns 用户任务作品列表
   */
  async getUserTaskPosts(payload: {
    accountId?: string
    platform?: string
    uid?: string
    taskId?: string
    ...
    try {
      const userTaskPostsModel = this.getUserTaskPostsModel()

      // 构建查询条件
      const filter: RootFilterQuery<UserTaskPosts> = {}

      if (payload.accountId) {
        filter.accountId = payload.accountId
      }
      if (payload.platform) {
        filter.type = payload.platform as AccountType
      }
      if (payload.uid) {
      ...
      if (payload.postId) {
        filter.postId = payload.postId
      }

      // 查询数据
      const results = await userTaskPostsModel.find(filter).lean()

      this.logger.debug(`Found ${results.length} user task posts with filter: ${JSON.stringify(filter)}`)

      return results
    }
    catch (error) {
      this.logger.error(`Error getting user task posts: ${error}`)
      throw error
    }
  }
   ...
   * @returns 用户任务作品列表
   */
  async getUserTaskPostsByTaskId(taskId: string) {
    return this.getUserTaskPosts({ taskId })
  }
   ...
   * @returns 用户任务作品列表
   */
  async getUserTaskPostsByUserId(userId: string) {
    return this.getUserTaskPosts({ userId })
  }
   ...
   * @returns 用户任务作品信息
   */
  async getUserTaskPostByPostId(postId: string, platform: string) {
    const results = await this.getUserTaskPosts({ postId, platform })
