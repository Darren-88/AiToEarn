import qs from 'qs'
import { checkMetaAuthApi, getInstagramAuthUrlApi } from '@/api/platAuth'
 ...
 * @param platType
 */
export async function instagramSkip(platType: PlatType, spaceId?: string) {
  if (platType !== PlatType.Instagram)
    return

  const res: any = await getInstagramAuthUrlApi('pc', spaceId)
  if (res?.code == 1) {
  ...
  return await instagramLogin(res.data.taskId)
}
